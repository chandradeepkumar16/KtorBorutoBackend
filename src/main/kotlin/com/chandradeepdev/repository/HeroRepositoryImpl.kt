package com.chandradeepdev.repository

import com.chandradeepdev.models.ApiResponse
import com.chandradeepdev.models.Hero
import java.util.*
...
class HeroRepositoryImpl: HeroRepository {
    ...
    override suspend fun getAllHeroes(page: Int): ApiResponse {
        return ApiResponse(
            success = true,
            message = "ok",
            previousPage = calcPage(page)[PREV_PAGE_KEY],
    ...
    private fun findHeroes(query :String?):List<Hero>{
        ...
        return if(!query.isNullOrEmpty()){
            heroes.forEach{( _, heroes) ->
                heroes.forEach { hero ->
                    if(hero.name.lowercase(Locale.getDefault()).contains(query.lowercase(Locale.getDefault()))){
                        founded.add(hero)
                    }
                }
            }
